...
说明：
- 新增自动路由接口 `route_infer` 与坐姿占位接口 `posture/analyze` 后，继续复用该集合记录路由决策与访问结果（可在 `endpoint` 与 `result` 字段中区分）。
- `route_infer` 的 `route_mode`（auto/force_bone/force_posture）可作为路由审计字段的一部分记录在日志摘要中。
...
- 2026-04-22：新增自动路由与坐姿占位接口，确认复用`security_access_logs`审计，无需新增集合（v1.5）
- 2026-04-22：自动路由支持`route_mode`前端偏好参数，确认无需新增集合（v1.6）
