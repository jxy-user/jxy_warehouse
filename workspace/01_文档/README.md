...
- 演示页面：`../02_工程/frontend_dark.html`
- 用途：路演展示、接口联调入口、深色风格系统首页原型
- 新增：支持上传图片并在线调用 `POST /infer_with_image`，展示风险分数表与热图预览
- 新增：支持自定义接口基地址（默认 `http://127.0.0.1:8000`），可切换本地/云端服务
- 新增：提供“最近3个接口地址”历史下拉，支持快速切换