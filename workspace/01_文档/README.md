...
- **路线图（并行能力，尚未在默认工程中启用）**：与 `route_infer` **并行**的受控问答服务（科普、操作指引、非诊断对话），规划接口如云函数 `qa-assistant` 或 `POST /chat`，由前端与影像分析分流；详见 `docs/12_灵枢AI项目计划书（详细版）.md` §7.4、`docs/11_灵枢AI项目计划书.md` §7.5
